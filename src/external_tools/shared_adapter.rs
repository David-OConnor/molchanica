...
use std::{collections::HashMap, fs, io, path::{Path, PathBuf}, process::Command, time::{SystemTime, UNIX_EPOCH}};

use serde_json::{Map, Value};

use super::{Tool, data_root, tool_form::{FieldKind, FormContract}};
...
pub fn payload(tool: Tool, values: &HashMap<String, String>, mode: &str) -> io::Result<Value> {
    ...
    for field in &contract.fields {
        if !field.applies_to(mode) { continue; }
        let Some(value) = values.get(&field.name) else { continue; };
        let mut value = value.clone();
        if field.kind() == FieldKind::File && !value.trim().is_empty() && !value.starts_with("bio-tools://") {
            value = fs::read_to_string(value.trim()).map_err(|error| io::Error::other(format!("{}: {error}", field.label)))?;
        }
        result.insert(field.name.clone(), Value::String(value));
    }
    if !mode.is_empty() { result.insert("input_mode".into(), Value::String(mode.into())); }
    if let Some(task) = values.get("task") { result.insert("task".into(), Value::String(task.clone())); }
...
pub fn run(tool: Tool, payload: Value, control: &PredictionControl) -> io::Result<AdapterResult> {
    if !tool.spec().platform.is_supported() {
        return Err(io::Error::other(format!("{} requires Linux", tool.spec().name())));
    }
    let root = data_root().ok_or_else(|| io::Error::other("Cannot find the Molchanica data folder"))?.join("process_executables");
    let stamp = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_nanos();
    let directory = root.join("results").join(slug(tool)).join(format!("{stamp}-{}", std::process::id()));
    ...
    let uv = std::env::var_os("MOLCHANICA_UV").or_else(|| std::env::var_os("BIO_TOOLS_UV")).unwrap_or_else(|| "uv".into());
    ...
    if control.is_cancel_requested() { return Err(io::Error::new(io::ErrorKind::Interrupted, "Run cancelled")); }
    let envelope: Value = fs::read(&response).ok().and_then(|bytes| serde_json::from_slice(&bytes).ok()).unwrap_or(Value::Null);
    ...
    execution.map_err(|error| io::Error::other(format!("{error}\nRun files: {}", directory.display())))?;
    let details = envelope.get("result").cloned().ok_or_else(|| io::Error::other("The adapter did not return results"))?;
    let log = details.get("run_log_dir").and_then(Value::as_str).ok_or_else(|| io::Error::other("Missing results directory"))?;
    let mut files = Vec::new();
    collect_files(&Path::new(log).join("outputs"), &mut files)?;
    files.sort();
    Ok(AdapterResult { archive: directory.join("raw-results.zip"), directory, files, details })
}
...
fn collect_files(path: &Path, files: &mut Vec<PathBuf>) -> io::Result<()> {
    ...
    for entry in fs::read_dir(path)? {
        ...
        if kind.is_dir() { collect_files(&entry.path(), files)?; }
