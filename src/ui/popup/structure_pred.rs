...
impl Default for StructurePredUi {
    fn default() -> Self { Self { window: ToolWindow::new(Tool::OpenDde), message: None, } }
}
...
pub(in crate::ui) fn structure_prediction_window(state: &mut State, scene: &mut Scene, updates: &mut EngineUpdates, ui: &mut Ui) {
    ui.horizontal(|ui| { ui.heading("Structure prediction and co-folding"); close_btn(ui, &mut state.ui.popup.structure_pred); });
    if let Some(message) = &state.ui.structure_pred.message { ui.label(message); }
    let load = state.ui.structure_pred.window.draw(&[Tool::OpenDde, Tool::Boltz2, Tool::Chai1, Tool::EsmFold2], &state.peptides, ui);
